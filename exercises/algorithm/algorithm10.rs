...
基于测试用例分析，我们需要实现以下功能：
add_node - 为图添加一个节点
add_edge - 为图添加一条边，由于是无向图，需要双向添加边
修改计划：
实现 add_node 方法，确保如果节点已存在则返回 false，否则添加节点并返回 true
在 Graph trait 和 UndirectedGraph 结构体中实现 add_edge 方法，无向图需要添加双向边
注意在 Graph trait 中的 add_edge 方法已经变成了抽象方法（没有方法体）
